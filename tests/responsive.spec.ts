...
test.describe('Responsive Design', () => {
  ...
  test('Mobile viewport (375px) - iPhone SE', async ({ page }) => {
    ...
    const desktopNavLinks = page.locator('nav a').filter({ hasText: /Journey|Now|Projects|Contact/ });
    ...
    const heroTitle = page.getByText('CHAIM LEV-ARI');
  ...
  test('Tablet viewport (768px) - Responsive layout at tablet breakpoint', async ({ page }) => {
  ...
  test('Desktop viewport (1920px) - Full desktop layout', async ({ page }) => {
    ...
    const navJourney = page.locator('nav a').filter({ hasText: /Journey/i }).first();
    const navProjects = page.locator('nav a').filter({ hasText: /Projects/i }).first();
    const navContact = page.locator('nav a').filter({ hasText: /Contact/i }).first();
  ...
  test('No horizontal overflow - Verify at multiple viewport sizes', async ({ page }) => {
    const viewports = [
    ...
    for (const viewport of viewports) {
      ...
      const htmlScrollWidth = await page.locator('html').evaluate(el => el.scrollWidth);

      // Allow 1px tolerance for rounding issues
      expect(bodyScrollWidth, `${viewport.name}: Body scroll width should not exceed viewport`).toBeLessThanOrEqual(viewport.width + 1);
      expect(htmlScrollWidth, `${viewport.name}: HTML scroll width should not exceed viewport`).toBeLessThanOrEqual(viewport.width + 1);
      ...
      const bodyScrollWidthAfter = await page.locator('body').evaluate(el => el.scrollWidth);
      expect(bodyScrollWidthAfter, `${viewport.name}: No overflow after scrolling`).toBeLessThanOrEqual(viewport.width + 1);
    }
  });

  test('Touch targets on mobile - Verify 44x44px accessibility standard', async ({ page }) => {
    ...
    expect(themeToggleBox!.width, 'Theme toggle width should be at least 44px').toBeGreaterThanOrEqual(44);
    expect(themeToggleBox!.height, 'Theme toggle height should be at least 44px').toBeGreaterThanOrEqual(44);
    ...
    expect(viewProjectsBtnBox!.height, 'View Projects button height should be at least 44px').toBeGreaterThanOrEqual(44);
    ...
    expect(getInTouchBtnBox!.height, 'Get In Touch button height should be at least 44px').toBeGreaterThanOrEqual(44);
    ...
    if (await githubLink.count() > 0) {
      ...
      expect(githubLinkBox!.width, 'GitHub link width should be at least 44px').toBeGreaterThanOrEqual(44);
      expect(githubLinkBox!.height, 'GitHub link height should be at least 44px').toBeGreaterThanOrEqual(44);
    }
    ...
    if (await linkedinLink.count() > 0) {
      ...
      expect(linkedinLinkBox!.width, 'LinkedIn link width should be at least 44px').toBeGreaterThanOrEqual(44);
      expect(linkedinLinkBox!.height, 'LinkedIn link height should be at least 44px').toBeGreaterThanOrEqual(44);
    }
    ...
    if (await emailLink.count() > 0) {
      ...
      expect(emailLinkBox!.width, 'Email link width should be at least 44px').toBeGreaterThanOrEqual(44);
      expect(emailLinkBox!.height, 'Email link height should be at least 44px').toBeGreaterThanOrEqual(44);
    }

    // CONTACT SECTION: Check contact card links
    await page.locator('#contact').scrollIntoViewIfNeeded();

    const contactEmailCard = page.locator('#contact').locator('a[href^="mailto:"]').first();
    if (await contactEmailCard.count() > 0) {
      ...
      expect(contactEmailCardBox!.height, 'Contact email card should be tappable').toBeGreaterThanOrEqual(44);
    }
  });
});
