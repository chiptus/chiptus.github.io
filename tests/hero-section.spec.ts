...
test.describe('Hero Section', () => {
  test('Hero section renders correctly', async ({ page }) => {
    ...
    await expect(page.getByText('FULL-STACK DEVELOPER', { exact: false })).toBeVisible();
    ...
    await expect(page.getByText(/Full-stack developer with 10\+ years of experience/)).toBeVisible();
  ...
  test('CTA buttons are functional', async ({ page }) => {
    ...
    const viewProjectsBtnRefreshed = page.getByRole('link', { name: /View Projects/i });
    const getInTouchBtnRefreshed = page.getByRole('link', { name: /Get In Touch/i });

    // View Projects button should have shadow effect
    const viewProjectsBtnClass = await viewProjectsBtnRefreshed.getAttribute('class');
    expect(viewProjectsBtnClass).toContain('shadow-brutal');

    // Get In Touch button should have outline variant
    const getInTouchBtnClass = await getInTouchBtnRefreshed.getAttribute('class');
  ...
  test('Social links work', async ({ page }) => {
    ...
    const githubLink = page.getByRole('link', { name: 'GitHub' });
    await expect(githubLink).toBeVisible();
    await expect(githubLink).toHaveAttribute('href', 'https://github.com/chiptus');
    ...
    const linkedinLink = page.getByRole('link', { name: 'LinkedIn' });
    await expect(linkedinLink).toBeVisible();
    await expect(linkedinLink).toHaveAttribute('href', 'http://linkedin.com/in/chiptus');
